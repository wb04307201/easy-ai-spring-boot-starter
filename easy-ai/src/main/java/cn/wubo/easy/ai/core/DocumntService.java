...
public class DocumntService {
    ...
    public DocumntService(IDocumentStorageService documentStorageService, IDocumentStorageRecord documentStorageRecord, IDocumentReaderService documentReaderService, VectorStore vectorStore) {
    ...
    public DocumentStorageDTO upload(InputStream is, String fileName) {
        ...
        documentStorageDTO.setFilePath(documentStorageService.save(is, documentStorageDTO.getFileName()));
        // 设置文件存储状态为"00"，表示存储成功
        documentStorageDTO.setState("00");
        // 设置文件的创建时间为当前时间
        documentStorageDTO.setCreateTime(new Date());
        // 将文件存储DTO保存到文件存储记录中，并返回保存后的DTO
        return documentStorageRecord.save(documentStorageDTO);
    }
    ...
    public DocumentStorageDTO read(DocumentStorageDTO documentStorageDTO) {
        ...
        documentStorageDTO.setDocumentList(documentReaderService.read(documentStorageService.getResource(documentStorageDTO.getFilePath())));
        // 更新文件存储状态为"20"，表示处理完成
     ...
     * 接下来，方法调用documentStorageRecord的save方法来保存文件存储信息，并更新fileStorageDTO对象。
     * 之后，方法将文件内容接受到vectorStore中，这一步可能是将文件内容索引到一个矢量数据库中以供后续查询。
     * 最后，方法更新文件的状态为"40"，表示文件存储过程已经完成，并再次调用documentStorageRecord的save方法来保存更新后的文件存储信息。
     *
    ...
    public DocumentStorageDTO save(DocumentStorageDTO documentStorageDTO) {
        ...
        documentStorageDTO = documentStorageRecord.save(documentStorageDTO);
        // 将文件内容接受到矢量存储中
        vectorStore.add(documentStorageDTO.getDocumentList());
        // 更新文件存储状态为"40"，表示处理完成
    ...
    public Boolean delete(String id) {
        // 根据ID查找文件存储记录。
        DocumentStorageDTO documentStorageDTO = documentStorageRecord.findById(id);

        // 删除文件存储DTO中包含的所有文档。
        vectorStore.delete(documentStorageDTO.getDocumentList().stream().map(Document::getId).toList());

        // 从文件存储记录中删除相应的记录。
        documentStorageRecord.delete(documentStorageDTO);

        // 删除文件存储服务中的实际文件。
        documentStorageService.delete(documentStorageDTO.getFilePath());
    ...
    public DocumentStorageDTO findById(String id) {
        return documentStorageRecord.findById(id);
    }

    /**
     * 列出文件存储信息。
     * <p>
     * 本方法通过调用documentStorageRecord类的list方法，来获取并返回文件存储的相关信息。
    ...
    public List<DocumentStorageDTO> list(DocumentStorageDTO documentStorageDTO) {
        return documentStorageRecord.list(documentStorageDTO);
    }
    ...
    public byte[] getBytes(DocumentStorageDTO documentStorageDTO) {
        ...
        return documentStorageService.getBytes(documentStorageDTO.getFilePath());
    }
}
